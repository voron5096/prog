...
Если общее описание вам оказалось не совсем понятным, рассмотрим его на конкретном примере. Представим что у нас есть неотсортированный массив числовых значений. Введём такой замечательный человеческий орган как глаз, и представим что он показывает где в данный момент времени находится компилятор. Весь процесс состоит из двух глобальных этапов. Первый из них - нечетный проход.
<br>
...
Что же происходит когда все элементы отсортированы, и как вообще компилятору понять что пора останавливаться? Достаточно запустить нечетный проход и понять что перестановок между числами не произошло. И тоже самое провести с четными элементами. И если замен два раза не произошло можно понять что сортировку пора завершать.
